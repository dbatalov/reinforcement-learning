 ...
 * TODO make this an Observable class in true spirit of MVC
 * @author denisb
 */
...
//	private static final Point2D GRAVITY = new Point2D.Float(0, -1.625f); // Moon, m/s^2
	...
	*/
	...
	public RocketLander() {
		this.reset((float)this.getPosition().getX());
	}

	public RocketLander(final RocketLander toCopy) {
		this.setFrom(toCopy);
	}

	public void setFrom(final RocketLander source) {
		this.position = new Point2D.Float((float)source.position.getX(), (float)source.position.getY());
		this.velocity = new Point2D.Float((float)source.velocity.getX(), (float)source.velocity.getY());
		this.rotation = source.rotation;
		this.burnLeft = source.burnLeft;
		this.burnRight = source.burnRight;
	...
	/**
	 * @return true if rocket is landed, false otherwise
	 */
	...
	public boolean isLandedOrCrashed() {
		return this.crashed() || this.checkLanded();
	}
	...
	public static void main(final String[] args) {
		...
		for (int t = 0; t < 10000; t++) {
			...
			lander.tick(0.1f); // tenth of a second
